...
class AgentConfig(BaseModel):
    ...
    #   ``claude_agent_sdk`` — ``ClaudeSDKClient`` (OAuth-authenticated),
    #       traffic drawn against the monthly Agent SDK credit. Peripherals
...
class ApiKeysConfig(BaseModel):
    ...
    # Long-lived OAuth token (1yr) generated by `claude setup-token` on a
    # machine with a browser. When ``agent.backend == "claude_agent_sdk"``
    # this drives the Agent SDK subprocess's auth and bills against the
    # monthly Agent SDK credit instead of the API. Peripherals (memory
    # rerank, batch pollers, web_search firewall, photo tagging) still
    # read ``anthropic`` and bill via the API key — that's intentional.
